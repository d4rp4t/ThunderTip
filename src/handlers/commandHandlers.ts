...
export async function handleStart(ctx: CommandContext<MyContext>) {
	await ctx.reply(
		"Hey there! I'm ThunderTip, your friendly Telegram bot for quick and easy bitcoin transactions, using Nostr Wallet Connect. Need help? Just hit /help.",
	);
}

export async function handleHelp(ctx: CommandContext<MyContext>) {
	await ctx.reply(
		"Hey! I'm your go-to bot for fast bitcoin transactions in Telegram chats.\n\n" +
			"Commands:\n\n" +
...
export async function handleNwcInfo(ctx: CommandContext<MyContext>) {
	await ctx.reply(
		"NWC or Nostr Wallet Connect is standarized protocol allowing developers to create secure and non-custoidal lightning apps in standarized way. Connection usually works via conection link generated by user in his wallet. Read more at https://github.com/getAlby/nostr-wallet-connect",
	);
}
...
export async function handleZap(ctx: CommandContext<MyContext>) {
	try {
		...
		if (args.length === 2) {
			...
			tipAmount === 0 &&
				(() => {
					throw new ZeroValueTipError("");
				})();
			tipAmount < 0 &&
				(() => {
					throw new NegativeTipAmountError("");
				})();

			if (tipAmount > 0) {
				(!tipReceiverString || isNaN(tipAmount)) &&
					(() => {
						throw new CommandSyntaxError("");
					})();
				await handleTransaction(tipReceiver, sender, tipAmount, ctx);
			}
...
export async function handleBalance(ctx: CommandContext<MyContext>) {
	try {
		...
		const balance = await user.connection?.nwcClient.getBalance();
		if (ctx.msg) {
			//try to respond on a message - helps when group chats is divided on topics
			await ctx.reply(
				`Your balance is ${balance.balance} ${balance} sat. Treat yourself!`,
				{
