...
class NwcValidationError extends Error {
	constructor(message: string) {
		super(message);
	}
}

class NwcPaymentError extends Error {
	constructor(message: string) {
		super(message);
	}
}

class NwcConnectionError extends Error {
	constructor(message: string) {
		super(message);
	}
}

class InvalidNwcUrlError extends Error {
	constructor(message: string) {
		super(message);
	}
}
...
async function handleError(error: Error, ctx: Context) {
	...
	if (error instanceof CommandSyntaxError) {
		message =
			"Oops, something went wrong! Please use the correct syntax: /tip <username> <amount>. For example: /tip @username 10";
	}
	...
	if (error instanceof SenderNotConnectedError) {
		message =
			"Sender isn't connected to the bot, so the transaction can't be processed. \nTip wasn't sent.";
	}
	...
	if (error instanceof NwcValidationError) {
		message =
			"NWC connection validation failed. Please check your NWC URL and try again.";
	}
	if (error instanceof NwcPaymentError) {
		message = "Payment failed. Please check your wallet balance and connection.";
	}
	if (error instanceof NwcConnectionError) {
		message = "NWC connection error. Please verify your wallet is online and try again.";
	}
	if (error instanceof InvalidNwcUrlError) {
		message = "Invalid NWC URL format. Please provide a valid Nostr Wallet Connect URL.";
	}
	if (message) {
		if (ctx.msg) {
